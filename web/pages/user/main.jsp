		...
		<div class="container">
			...
			<div class="contenido">
				...
				<table class="results_width_user results">
					<tbody>
						<tr>
							<th colspan="5" style="text-align: center;">Servicios de
								Información</th>
						</tr>
						<tr>
						...
						<s:if test="ListaServicios.size() > 0">
							<s:iterator value="ListaServicios" status="result_datos">
								<tr id="node-<s:property value="#result_datos.index"/>">
									...
									<td><s:set name="estado" value="servicio.id_estado"></s:set>
										<s:iterator value="estados">
											<s:if test="#estado == id_estado">
												<s:property value="nombre" />
											</s:if>
										</s:iterator></td>
									<td><s:date name="servicio.fecha_creado"
											format="d'/'MM'/'yyyy" /></td>
									<td>
										<table style="margin: 0; padding: 0;">
											<tr style="margin: 0; padding: 0;">
												<td style="margin: 0; padding: 0;"><s:set var="i"
														value="#result_datos.index"></s:set>
												...
												<td style="margin: 0; padding: 0;">
													<form action="eliminarServicioInformacion" method="POST"
														...
														<input type="submit" value="<s:text name="eliminar" />"
															style="font-size: 0.9em;"
															onclick="eliminar_SI(<s:property value="#i"/>,'<s:property value="servicio.nombre" />');" />
													</form>
												</td>
												<s:if test="publicable==true">
													<td style="margin: 0; padding: 0;"><s:if
															test="servicio.publicado==false">
															<form action="publicarServicioInformacion" method="POST"
																id="id_pub_<s:property value="#result_datos.index"/>"
																onsubmit="return false;">
																<s:hidden name="id_servicio_informacion"
																	value="%{servicio.id_servicio_informacion}"></s:hidden>
																<input type="submit" style="font-size: 0.9em;"
																	value="<s:text name="publicar"/>"
																	onclick="publicar_SI(<s:property value="#i"/>,'<s:property value="servicio.nombre" />');" />
															</form>
														</s:if> <s:else>
															<form action="despublicarServicioInformacion"
																method="POST"
																id="id_despub_<s:property value="#result_datos.index"/>"
																onsubmit="return false;">
																<s:hidden name="id_servicio_informacion"
																	value="%{servicio.id_servicio_informacion}"></s:hidden>
																<input type="submit"
																	value="<s:text name="despublicar" />"
																	style="font-size: 0.9em;"
												...
												<s:else>
													<s:set name="action"
														value="%{'prepararModificarServicioInformacion'}"></s:set>
													<s:if test="incompletos.size()==1">
														<s:i18n name="ve/gob/cnti/srsi/i18n/errors">
															<s:iterator value="incompletos">
																...
																<s:set name="i18n_funcionalidades"
																	value="%{getText('error.servicio.incomplete.funcionalidades')}"></s:set>
																<s:set name="i18n_salidas"
																	value="%{getText('error.servicio.incomplete.salidas')}"></s:set>
																<s:if
																	test="%{(#error == #i18n_salidas) || (#error == #i18n_funcionalidades)}">
																	<s:set name="action"
																		value="%{'prepararFuncionalidades'}"></s:set>
																</s:if>
															</s:iterator>
														</s:i18n>
													</s:if>
													...
													<td style="margin: 0; padding: 0;">
														<form action='<s:property value="#action" />'
															...
															<s:hidden name="id_servicio_informacion"
																value="%{servicio.id_servicio_informacion}"></s:hidden>
															<input type="submit"
																value="<s:text name="continuar_registro" />"
																style="font-size: 0.9em;" />
														</form>
													</td>
													<td style="margin: 0; padding: 0;"><img
														src="res/img/important.png"
														id="h<s:property value='#result_datos.index'/>"
														alt="ayuda" onmouseover="tip(this);"
														name="h<s:property value='#result_datos.index'/>"
														height="25" width="30" />
														<div class="h<s:property value='#result_datos.index'/>"
